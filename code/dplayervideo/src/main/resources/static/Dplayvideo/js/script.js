window.onload = function () {
	...
	dl.style.transform = "rotateX(-13deg) rotateY(0deg)";
	...
	window.onmousedown = function (e) {
		...
		window.onmousemove = function (e) {
			...
			dl.style.transform = "rotateX(-13deg) rotateY(" + y + "deg)";//修改每次传入的倾斜角度，使其固定只能按一个角度运动
		}
	...
	function deal(dds, n) {
		...
		var time = setInterval(function () {
			...
			translateZ += translateZTerminus / speed * 5;//加载旋转速度
			rotateY += angle / speed * 10;//加载飞出速度
